...
export default function AllLeads() {
  ...
  async function fetchLeads() {
    ...
    try {
      const res = await api.get('/admin/getCreatedLeads');
  ...
  async function markAssigned(id: number) {
    try { await api.patch(`/admin/markLeadAsAssigned/${id}`); setLeads(p=>p.map(l=>l.enquiryId===id?{...l,enquiryStatus:'ASSIGNED'}:l)); }
    catch(e){ console.error(e); alert('Failed to mark assigned'); }
  }
  async function markRemoved(id: number) {
    try { await api.patch(`/admin/markLeadAsRemoved/${id}`); setLeads(p=>p.map(l=>l.enquiryId===id?{...l,enquiryStatus:'REMOVED'}:l)); }
    ...
    return (
      <div className="rounded-xl shadow-md border-2 border-orange-100 bg-white overflow-hidden h-full flex flex-col">
        <div className="relative">
          ...
          <span className={["absolute left-3 top-3 inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-semibold shadow-sm", isRes? 'bg-amber-50 text-amber-700' : 'bg-sky-50 text-sky-700'].join(' ')}>
        ...
        <div className="p-3 flex flex-col flex-1 min-h-0">
          ...
          <div className="mt-4 flex items-center justify-between">
            <div>
              <div className="text-orange-600 text-lg font-bold">{priceLabel}</div>
            </div>
                      ...
                      <div className="p-3 flex flex-col flex-1 min-h-0">
                        ...
                        <div className="mt-3 flex gap-2">
                          <button
                            ...
                            className={`flex-1 px-3 py-2 rounded text-sm font-medium transition-colors border ${lead.enquiryStatus==='ASSIGNED' ? 'bg-gray-200 text-gray-600 border-gray-200' : 'bg-green-500 hover:bg-green-600 border-green-500 text-white'}`}
                          >
                            {lead.enquiryStatus==='ASSIGNED' ? 'Assigned' : 'Assign to PropAdda team'}
                          </button>

                          <button
                            onClick={()=>{ if(confirm('Remove lead?')) markRemoved(lead.enquiryId); }}
                            disabled={lead.enquiryStatus==='REMOVED'}
                            className={`flex-1 px-3 py-2 rounded text-sm font-medium transition-colors border ${lead.enquiryStatus==='REMOVED' ? 'bg-gray-200 text-gray-600 border-gray-200' : 'bg-red-500 hover:bg-red-600 border-red-500 text-white'}`}
                          >
                            Remove
                          </button>
                        </div>
