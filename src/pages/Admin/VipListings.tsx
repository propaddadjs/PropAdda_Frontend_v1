        ...
        <div className="mt-auto">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {p.vip && (
                <div className="flex items-center gap-2 text-sm font-semibold text-yellow-800 bg-yellow-200 px-2 py-0.5 rounded">
                  <Star className="w-4 h-4" /> VIP
                </div>
              )}
              {p.adminApproved && (
                <div className="flex items-center gap-2 text-xs font-semibold text-green-700 bg-green-50 px-2 py-0.5 rounded">
          ...
          {/* Actions stacked for mobile friendliness */}
          <div className="mt-3 flex flex-col gap-2">
            <Link
              to={`/admin/listings/view/${p.category ?? "Residential"}/${p.listingId}`}
              className="w-full px-3 py-2 text-white rounded text-sm bg-emerald-600 flex items-center justify-center gap-2"
            >
            ...
            <button
              ...
              className="w-full px-3 py-2 text-white rounded text-sm bg-rose-600 flex items-center justify-center gap-2"
            >
    ...
    <div className="flex">
      ...
      <main className="flex-1 p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {pageSlice.length === 0 && !loading && <div className="text-gray-600 col-span-1">No VIP Listings found.</div>}

          {[...pageSlice]
            .sort((a, b) => (b?.listingId ?? 0) - (a?.listingId ?? 0))
            .map((p) => (
              <div key={p.listingId} className="w-full">
                <Card p={p} onUnmarkVip={unmarkVip} />
              </div>
            ))}
        </div>
