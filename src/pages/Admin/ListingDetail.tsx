...
const ListingDetail: React.FC = () => {
  ...
  const imagesAll = useMemo(() => {
    const imgs = (allMedia || [])
      .filter((m: MediaResponse) => isImageUrl(m.url))
      .sort((a: any, b: any) => (a.ord ?? 999) - (b.ord ?? 999));
    return imgs.slice(0, 10); // allow up to 10 images
  }, [allMedia]);

  const videosAll = useMemo(() => {
    const vids = (allMedia || [])
      .filter((m: MediaResponse) => isVideoUrl(m.url))
      .sort((a: any, b: any) => (a.ord ?? 999) - (b.ord ?? 999));
  ...
  const brochuresAll = useMemo(() => {
    const bros = (allMedia || [])
      .filter((m: MediaResponse) => isPdf(m.url, m.filename))
      .sort((a: any, b: any) => (a.ord ?? 999) - (b.ord ?? 999));
  ...
  return (
    <div className="w-full box-border bg-gray-50 min-h-screen">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <div className="min-w-0">
                <h1 className="text-lg sm:text-2xl md:text-3xl font-semibold leading-tight break-words">{property.title ?? "Untitled Property"}</h1>
                <div className="text-xs sm:text-sm text-gray-500">ID: {property.listingId}</div>
              </div>
            </div>
              ...
              {property.sold ? (
                ...
                <>
                  {!property.expired && property.adminApproved && (
                    property.adminApproved.toLowerCase() === "pending"
                      ? pill(<><Hourglass className="h-3.5 w-3.5"/> Pending</>, "bg-yellow-50 text-yellow-600")
                      : property.adminApproved.toLowerCase() === "approved"
      ...
      {/* Reject modal */}
      {showRejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 sm:px-6" aria-modal="true" role="dialog">
          <div
            className="absolute inset-0 bg-black/40 transition-opacity"
            aria-hidden="true"
          />
          <div
            className="relative border-2 border-solid bg-white w-full max-w-xl rounded shadow-lg p-4 sm:p-6 z-50 max-h-[85vh] overflow-y-auto"
